<template>
    <div class="users-table">
     <md-card>
      ...
      <md-card-content>
            <md-table v-if="products.length>0" v-model="products" class="products-table" md-sort="name" md-sort-order="desc">
                <md-table-toolbar>
            ...
            <strong v-else class="text-danger">No Products registed yet .</strong>
      </md-card-content>
     </md-card>
  </div>
</template>
...
export default {
    ...
    methods: {
     async deleteProduct(id)
      {
        //console.log(id);
        await axios.get('/admin/removeProduct', { params: { id: id }}).then(res => this.products= res.data)//this.$emit("remove-location", res.data))
      }
    },
    created(){
      axios.get('/admin/products').then(res => this.products= res.data).catch(err => console.log(err))
    }
